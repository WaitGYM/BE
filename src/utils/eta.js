...
function calculateRealTimeETA(usage) {
  ...
  const remainingSets = Math.max(
    0,
    (usage.totalSets ?? 0) - (usage.currentSet ?? 0) + 1,
  );

  if (usage.setStatus === "EXERCISING") {
    const elapsed = usage.currentSetStartedAt
      ? now - new Date(usage.currentSetStartedAt).getTime()
      : 0;
  ...
  if (usage.setStatus === "RESTING") {
    const restElapsed = usage.restStartedAt
      ? now - new Date(usage.restStartedAt).getTime()
      : 0;
