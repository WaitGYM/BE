...
router.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (include_status === "true") {
      statusMap = await getEquipmentStatusInfo(
        list.map((e) => e.id),
        userId
      );
    }
    ...
    const response = list.map((e) => {
      const base = {
        ...
        isFavorite:
          !!userId && Array.isArray(e.favorites) && e.favorites.length > 0,
      };
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const statusMap = await getEquipmentStatusInfo(
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (!equipmentIds) {
      return res
        .status(400)
        .json({ error: "equipmentIds 파라미터가 필요합니다" });
    }
    ...
    if (ids.length === 0) {
      return res
        .status(400)
        .json({ error: "유효한 equipmentIds가 필요합니다" });
    }
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    stats.forEach((u) => {
      ...
      if (u.startedAt && u.endedAt) {
        ...
        equipmentStats[k].lastUsed =
          !equipmentStats[k].lastUsed || u.endedAt > equipmentStats[k].lastUsed
            ? u.endedAt
            : equipmentStats[k].lastUsed;
        totalSeconds += s;
      }
    ...
    res.json({
      ...
      totalSets,
      totalSeconds,
      averageSetsPerWorkout: stats.length
        ? Math.round(totalSets / stats.length)
        : 0,
      equipmentStats: Object.values(equipmentStats).sort(
        (a, b) => b.count - a.count
      ),
...
router.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (stats.workoutDetails.length > 0) {
      const longest = stats.workoutDetails.reduce(
        (max, workout) =>
